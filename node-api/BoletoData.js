...
async function fetchDbData(id, pool) {
  try {
    ...
    const payload = {
      ...
      viofPgtoTitlo: "0",
      filler2: "",
      isacdoTitloCobr: String(clienteNome ?? "").substring(0, 70),
      elogdrSacdoTitlo: String(enderecoContato ?? "").substring(0, 40),
      enroLogdrSacdo: String(numeroEnderecoContato ?? "").substring(0, 10),
      ecomplLogdrSacdo: String(complementoContato ?? "").substring(0, 15),
      ccepSacdoTitlo: cepContato ? String(cepContato).substring(0, 5) : "00000",
      ccomplCepSacdo: cepContato ? String(cepContato).slice(-3) : "000",
      ebairoLogdrSacdo: String(bairroContato ?? "").substring(0, 40),
      imunSacdoTitlo: String(municipio ?? "").substring(0, 30),
      csglUfSacdo: String(uf ?? "").substring(0, 2),
      indCpfCnpjSacdo:
        cnpjCpfCliente && String(cnpjCpfCliente).length === 11 ? "1" : "2",
      nroCpfCnpjSacdo: String(cnpjCpfCliente ?? "0"),
      renderEletrSacdo: String(emailContato ?? "").substring(0, 70),
      cdddFoneSacdo: telefoneContato
        ? String(telefoneContato).substring(0, 3)
        : "00",
      cfoneSacdoTitlo: String(telefoneContato ?? "0").substring(0, 11),
      bancoDeb: "000",
      agenciaDeb: "00000",
      agenciaDebDv: "0",
