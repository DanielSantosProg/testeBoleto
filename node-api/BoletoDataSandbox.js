...
async function fetchDbData(id, pool) {
  try {
    const dataSelect = await pool.request().input("id", sql.Int, id).query(`
      ...
      INNER JOIN API_BOLETO_CAD_CONVENIO CO ON D.COR_CLI_BANCO = CO.API_API_ID
    ...
    const payload = {
      ...
      qtdePgtoParcial: "000",
      filler1: "",
      ptxJuroVcto: modalidadeJuros === "P" ? String(juros ?? "0") : "0",
      vdiaJuroMora: modalidadeJuros === "V" ? String(juros ?? "0") : "0",
      qdiaInicJuro: "01",
      pmultaAplicVcto: tipoMulta === "P" ? String(multa ?? "0") : "000000",
      vmultaAtrsoPgto: tipoMulta === "V" ? String(multa ?? "0") : "0",
